function weights = DSICK_getDCompatCoeffs( d, nGhost, p, extrapOrd,BCD)
...
% example of usage
% weights = DSICK_getDCompatCoeffs(3, 9, 19, 20, [0,2,4,6])
...
ew = DSICK_getExtrapCoeffs( extrapOrd );
nw = length(ew);
...
for ig = ne+1:ng
  iid = iid+1;
  id = BCD(iid);
  dw = DSICK_generalDifferenceWeights(0,-(p-1)/2:(p-1)/2,id);
  nw = length(dw);
